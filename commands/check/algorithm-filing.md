...
2. 目标用户是否面向中国大陆 C 端 / 具舆论属性的 B 端？ `[A] 是` `[B] 仅海外` `[C] 仅内部 / 科研`
3. 产品主要能力类型？ `[A] 生成内容（文/图/音/视频）` `[B] 推荐 / 排序 / 检索` `[C] 深度合成（换脸 / 语音克隆 / 数字人）` `[D] 调度决策`
4. 已完成 Stage 2？ 营业执照经营范围是否含"人工智能技术开发 / 技术服务"？ `[Y/N/不确定]`
...
不管主轨是哪条，以下场景**额外叠加**一项备案：

- [ ] 含**人脸合成 / 换脸** → 叠加【深度合成算法备案】
- [ ] 含**语音合成 / 克隆** → 叠加【深度合成算法备案】
- [ ] 含**数字人 / 虚拟形象** → 叠加【深度合成算法备案】
- [ ] 含**图文 / 视频内容强编辑** → 叠加【深度合成算法备案】
- [ ] 同时上线欧盟 → 叠加【EU AI Act GPAI（自研）/ Art. 50（应用层）】（运行 `check/eu-ai-act-tier`）
- [ ] 训练 / 推理涉及出境 → 运行 `check/pipl-gap` 评估出境路径
...
**主轨道**：<A / B / C / 免备案>
**叠加项**：<深度合成 / EU AI Act / 出境 ... 或 无>

## 时间规划
| 里程碑 | 距离上线 T- |
|---|---|
| 第三方安评入场 | T-6 ~ T-9 月（仅 B 轨需要） |
| 材料齐全 | T-4 月 |
| 提交初审 | T-3 月 |
| 预期拿到批复 | T-0 ~ T+1 月 |
...
stage_5_output:
  ...
  filing_timeline_months: 8                    # 距离上线的提前量
  filing_target_launch_date: 2026-12-01        # 可选
  filing_red_flags:
