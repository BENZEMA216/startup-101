...
3. **阶段自诊断**（如果命令没明确指定 stage）：
   ...
   [1] 还没注册公司，纠结架构
   [2] 已决定架构，准备注册
   [3] 刚拿到执照，不知道接下来 30 天干啥（v0.2）
   [4] 要招人、发期权（v0.2）
   [5] AI 产品要上线，不懂合规（v0.2）
   [6] 准备融资
   [7] 已融完，要处理持续义务（v0.2）
   ```
   v0.1 可用：Stage 1 / 2 / 6。其他 stage 提示"v0.2 支持"。
...
- Red Flags（6 条）
- Appendix: entity-matrix, payment-matrix
- References index: incorporation, fundraising

## v0.2+ 待补

Stage 3 / 4 / 5 / 7，其他 appendix 和 references。
